 ...
 */
const sanitizeManimCode = (manimCode: string): string => {
...
export const renderManimScene = async (manimCode: string, sceneId: string, narrationText?: string): Promise<any> => {
  ...
  try {
    const response = await axios.post<ManimRenderResponse>(
      `${config.manimRenderService.endpoint}/render`,
      { 
    ...
    if (response.status === 400 && response.data?.error?.includes('Linting failed')) {
      ...
      throw new AppError(
        `Linting failed for Manim code in scene ${sceneId}: ${response.data.details_stdout || response.data.error}`,
        400, // Keep 400 status
    ...
    if (response.status === 500) {
      ...
      const importError = Object.keys(COMMON_IMPORT_FIXES).find(key => 
        stderrText.includes(`cannot import name '${key}'`) || 
      ...
      throw new AppError(
        `Manim rendering process failed for scene ${sceneId}: ${errorDetails}`,
        500, // Keep 500 status
        true, // Operational error
        {
 ...
 */
async function processVideoWithNarration(
  videoPath: string, 
