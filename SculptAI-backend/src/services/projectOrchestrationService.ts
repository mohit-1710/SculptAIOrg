...
export const initiateProjectAndStoryboard = async (
  userIdea: string,
  userId?: string
): Promise<{ projectId: string; storyboard: IStoryboardScene[] }> => {
...
export const processStoryboardToVideoScenes = async (
  projectId: string,
  storyboard: IStoryboardScene[],
  projectTopic: string = "User's Animation Project",
  userId?: string
  ...
  try {
    ...
    const processScene = async (sceneData: IStoryboardScene, sceneIndex: number): Promise<ISceneOutput> => {
      ...
      while (correctionAttempts <= MAX_CORRECTION_ATTEMPTS && !sceneProcessedSuccessfully) {
        try {
          ...
          const renderResult = await manimService.renderManimScene(
            manimCode, 
            `${projectId}_scene_${sceneNumber}`,
            sceneData.narration // Pass narration text for TTS
          );
          ...
          if (error instanceof AppError) {
            ...
            if (error.statusCode === 400 || error.statusCode === 500) {
              if (correctionAttempts <= MAX_CORRECTION_ATTEMPTS) {
                ...
                try {
                  manimCode = await llmService.correctManimCodeWithLLM(
                    manimCode,
                    error.details,
                    {
                      scene_number: sceneNumber,
          ...
          throw new AppError(
            `Failed to process scene ${sceneNumber} (${sceneData.scene_title})`,
            500,
            false,
            {
    ...
    const scenePromises = storyboard.map((scene, index) => 
      processScene(scene, index)
        .then(result => {
    ...
    let overallStatus: IProjectData['status'] = 'failed';
    ...
    throw new AppError(
      'Failed to process storyboard to video scenes',
      500,
      false,
      {
        originalError: error instanceof Error ? error.message : String(error),
